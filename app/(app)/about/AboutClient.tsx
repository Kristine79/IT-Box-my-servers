...
import { Shield, FolderKanban, Server, Network, KeyRound, Share2, Search, Smartphone } from 'lucide-react';
...
export default function AboutClient() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 pb-10">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="neu-panel p-6 md:p-8 rounded-2xl">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="neu-panel p-6 md:p-8 rounded-2xl">
...
function ModuleCard({ icon: Icon, title, description, delay, iconColor = "text-[var(--neu-accent)]", compact = false }: { icon: any, title: string, description: string, delay: number, iconColor?: string, compact?: boolean }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
      className={cn("neu-panel rounded-2xl flex items-center gap-4 transition-all hover:scale-[1.01]", compact ? "p-4" : "p-5 md:p-6")}>
      <div className={cn("neu-panel-inset rounded-xl shrink-0 flex items-center justify-center", compact ? "p-2.5" : "p-3.5", iconColor)}>
      ...
      <div>
        ...
        {!compact && <p className="text-[var(--neu-text-muted)] leading-tight text-xs md:text-sm">{description}</p>}
        {compact && description && <p className="text-[var(--neu-text-muted)] text-[11px] leading-none mt-0.5">{description}</p>}
      </div>
    </motion.div>
  );
}
...
function AudienceCard({ title, description, delay }: { title: string, description: string, delay: number }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
      ...
      <p className="text-[var(--neu-text-muted)] leading-relaxed text-xs md:text-sm flex-1">{description}</p>
    </motion.div>
  );
}
